...
| Pedidos delivery/retirada | `designs/gest_o_de_pedidos_delivery_retirada_forno_27/code.html` | Operar canais externos | abas, filtros, cards, novo pedido, PDF tabular, comanda térmica e avanço de status | Customer, Order, OrderItem, SalesChannel | A criação e o cadastro de cliente estendem a ação “Novo pedido” com os mesmos tokens do painel; valores são recalculados pelo servidor. |
...
| Backup e sistema | `designs/configura_es_backup_sistema_forno_27/code.html` | Operação técnica | backup automático/manual, histórico, informações do sistema | AuditLog | A tela executa e lista backups físicos com `pg_dump`; o snapshot JSON permanece auxiliar. |
| Fechamento de caixa | `designs/fechamento_de_caixa_forno_27/code.html` | Abrir, operar e fechar turno | abertura em modal, fundo inicial, conferência, valores, diferença, observações | CashRegister, CashShift, CashMovement, Payment | `screen.png` está corrompido pelo Stitch; a abertura reutiliza os tokens e padrões de modal do HTML e do painel integrado. |
