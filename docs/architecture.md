...
O mesmo token autentica `POST /api/v1/client/telemetry`. O tablet envia nível de bateria, carregamento, conectividade e versão a cada 30 segundos e quando esses valores mudam; a API determina o IP observado e delega ao agregado `Device` a validação do percentual entre 0 e 100. A listagem administrativa só considera um tablet online quando o último heartbeat tem no máximo dois minutos; assim, um IP de proxy Docker, como `172.18.0.1`, não é tratado como prova de presença. Navegadores que não oferecem a Battery Status API enviam bateria desconhecida sem inventar um valor.
